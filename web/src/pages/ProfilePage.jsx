          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1">
                ...
                {user.isVerified ? <span className="text-violet-500">🟣V</span> : null}
              </div>
              <p className="text-sm text-stone-500">{user.bio}</p>
            </div>
          </div>
          <div className="mt-3 flex items-center justify-between rounded-xl bg-violet-50 px-3 py-2 text-sm">
            ...
            <button
              type="button"
              onClick={logout}
              className="rounded-lg border border-violet-200 bg-white px-2 py-1 text-xs text-violet-700 active:scale-95"
            >
              退出登录
            </button>
          </div>
          ...
          {activeTab === "my-pets" ? (
            <div className="space-y-2">
              {myPets.length === 0 ? (
                ...
                myPets.map((pet) => (
                  <p key={pet.id} className="rounded-xl bg-violet-50 px-3 py-2 text-sm text-stone-700">
            ...
            <div className="space-y-2">
              {applications.length === 0 ? (
                ...
                applications.map((application) => (
                  <p
                    key={application.id}
                    className="rounded-xl bg-violet-50 px-3 py-2 text-sm text-stone-700"
                  >
        ...
        <article className="rounded-2xl bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-3">
            {revisitTimeline.map((item) => (
              <div key={item.month} className="rounded-xl bg-violet-50 p-2">
                <p className="mb-2 text-xs font-medium text-violet-700">{item.month}</p>
                <img
                  src={item.photo}
                  alt={item.month}
...
function EmptyHint({ text }) {
  return (
    <div className="rounded-xl border border-dashed border-violet-200 bg-violet-50 px-3 py-4 text-center">
